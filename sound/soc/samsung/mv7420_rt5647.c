...
static int espresso_7420_rt5647_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	pr_debug("%s() : OK\n", __func__);
	ear_detect=gpio_to_irq(of_get_named_gpio(np, "rt5647,ear_jack_detect_gpio", 0));
	if (ret< 0) {
		pr_err("Error in setting interrupt mode :%d int_gpio:%d\n", ret, of_get_named_gpio(np, "rt5647,ear_jack_detect_gpio", 0));
		goto  int_gpio_request_error;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(codec->dev, "set aif2 mode: %s\n",
...
static struct snd_soc_dai_driver mv7420_ext_dai[] = {
   {
       .name = "ap.cp",
       .playback = {
           .channels_min = 1,
...
static int mv7420_voice_hw_params(struct snd_pcm_substream *substream, 	
	struct snd_pcm_hw_params *params)
{	
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	...
	/* CLKOUT(XXTI) for rt5647 MCLK */
	espresso_enable_mclk(true);
//viola added for audio call @ 2014.8.12

	/*set clock*/
	pll_out = params_rate(params) *6*512;
	/* Set Codec DAI configuration */
	...
	if (ret < 0)
		return ret;
#if 1
	ret = snd_soc_dai_set_pll(codec_dai, RT5647_AIF3, RT5647_PLL1_S_MCLK,
					ESPRESSO_RT5647_AUD_PLL_FREQ, pll_out);
	if (ret < 0)
		return ret;
	ret = snd_soc_dai_set_sysclk(codec_dai, RT5647_SCLK_S_PLL1,
...
static int mv7420_BTSCO_hw_params(struct snd_pcm_substream *substream, 	
	struct snd_pcm_hw_params *params)
{	
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
//viola added for audio call @ 2014.8.12
	...
	/* Set Codec DAI configuration */
	ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_DSP_A|SND_SOC_DAIFMT_IB_NF
					// | SND_SOC_DAIFMT_CBM_CFM);
					 | SND_SOC_DAIFMT_CBS_CFS);
	...
	if (ret < 0)
		return ret;
#if 1
	ret = snd_soc_dai_set_pll(codec_dai, RT5647_AIF2, RT5647_PLL1_S_MCLK,
					ESPRESSO_RT5647_AUD_PLL_FREQ, pll_out);
	if (ret < 0)
		return ret;
	ret = snd_soc_dai_set_sysclk(codec_dai, RT5647_SCLK_S_PLL1,
...
static struct snd_soc_ops voice_call_ops = {
	.hw_params = mv7420_voice_hw_params,
};

static struct snd_soc_ops BTSCO_call_ops = {
	.hw_params = mv7420_BTSCO_hw_params,
};
...
static struct snd_soc_dai_link espresso_dai[] = {
	...
	}, { /* wnn: add for voice call*/
		.name = "RT5647 Voice Call",
		.stream_name = "Voice_Call_Dai",	
		.cpu_dai_name ="ap.cp",                 //"samsung-i2s.0" , //      
        .codec_dai_name = "rt5647-aif3",
        .platform_name = "snd-soc-dummy", /* use snd-soc-dummy */
        .ops = &voice_call_ops,
	}, {
		.name = "RT5647 BT Call",
		.stream_name = "BT_Call_Dai",	
		.cpu_dai_name ="ap.bt",                 //"samsung-i2s.0" , //      
...
#endif
	dev_dbg(card->dev, "%s: Successfully created\n", __func__);
...
{
	espresso_enable_mclk(false); /*Modified by Huangwenjian for sleep current leakage 20150417*/
	return 0;
}
...
static int espresso_audio_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_component(card->dev, &dummy_cmpnt, mv7420_ext_dai, ARRAY_SIZE(mv7420_ext_dai));
	if(ret)

...
#ifdef CONFIG_STORE_RTC_TIMER_LOG
#define RTC_DEBUG_LOGFILE "/data/rtc_log.txt"
#define RTC_GAP_DEBUG_LOGFILE "/data/rtc_gap_log.txt"
static struct workqueue_struct *rtc_debug_wq = NULL;
static struct work_struct rtc_debug_work;
static struct hrtimer rtc_debug_hrtimer;
static struct s2m_rtc_info *g_rtc_info = NULL;
static char rtc_log_buf[500];
int stop_timer_logging = 0;
#endif
...
#ifdef CONFIG_STORE_RTC_TIMER_LOG
static char* err_t_to_str[] = {"success", "failed to read time", "failed to write time"};
static char cur_rtc_buf[50];
static char set_rtc_buf[50];
#endif
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	int ret;
#ifdef CONFIG_STORE_RTC_TIMER_LOG
	u8 r_data[NR_RTC_CNT_REGS] = {0};
	struct rtc_time r_tm;
	struct file *fp = NULL;
	mm_segment_t pos = {0};
	int err;
	int err_t = 0;
	memset(cur_rtc_buf, 0, 50);
	memset(set_rtc_buf, 0, 50);
	ret = s2m_rtc_update(info, S2M_RTC_READ);
	if (ret < 0) {
		err_t = 1;
		goto out;
	...
	ret = sec_rtc_bulk_read(info->iodev, S2M_RTC_SEC, NR_RTC_CNT_REGS, r_data);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to read time reg(%d)\n", __func__,
			ret);
		err_t = 1;
	...
	s2m_data_to_tm(r_data, &r_tm);

	sprintf(cur_rtc_buf, "cur_rtc: %d-%02d-%02d %02d:%02d:%02d.\r\n",
			r_tm.tm_year + 1900, r_tm.tm_mon + 1, r_tm.tm_mday,
			r_tm.tm_hour, r_tm.tm_min, r_tm.tm_sec);

	sprintf(set_rtc_buf, "set_rtc: %d-%02d-%02d %02d:%02d:%02d.\r\n",
...
out:
	...
	fp = filp_open(RTC_DEBUG_LOGFILE, O_RDWR | O_APPEND | O_CREAT, 0666);
	if (IS_ERR(fp)) {
		printk("|RTC DEBUG | Failed to create log file |%s|, retry ...!\n", RTC_DEBUG_LOGFILE);
		fp = filp_open(RTC_DEBUG_LOGFILE, O_RDWR | O_APPEND | O_CREAT, 0666);
		if (IS_ERR(fp)) {
			printk("|RTC DEBUG | Failed to create log file |%s|, retry ...!\n", RTC_DEBUG_LOGFILE);
			set_fs(pos);
	...
	sprintf(rtc_log_buf, "=============================================\n|RTC SET TM| RTC time set to: %s, previously: %s. operation: %s\n", set_rtc_buf, cur_rtc_buf, err_t_to_str[err_t]);
	err = fp->f_op->write(fp, rtc_log_buf, strlen(rtc_log_buf), &fp->f_pos);
	printk("%s", rtc_log_buf);
...
#ifdef CONFIG_STORE_RTC_TIMER_LOG
static char rtc_buf[50];
static char sys_buf[50];
static int test_loop = 0;
static void s2m_rtc_debug_function(void)
{
	u8 r_data[NR_RTC_CNT_REGS] = {0};
	int ret;
	struct rtc_time r_tm, s_tm;
	struct timeval s_time;
	unsigned long r_time = 0, t_gap = 0;
	struct file *fp = NULL;
	mm_segment_t pos = {0};
	int err = 0;

	mutex_lock(&g_rtc_info->lock);

	memset(rtc_buf, 0, 50);
	memset(sys_buf, 0, 50);

	ret = s2m_rtc_update(g_rtc_info, S2M_RTC_READ);
	if (ret < 0) {
		goto out;
	}

	ret = sec_rtc_bulk_read(g_rtc_info->iodev, S2M_RTC_SEC, NR_RTC_CNT_REGS, r_data);
	if (ret < 0) {
		dev_err(g_rtc_info->dev, "%s: fail to read time reg(%d)\n", __func__,
			ret);
		goto out;
	}

	do_gettimeofday(&s_time);
	rtc_time_to_tm(s_time.tv_sec, &s_tm);
	sprintf(sys_buf, "cur_sys: %d-%02d-%02d %02d:%02d:%02d.\r\n",
			s_tm.tm_year + 1900, s_tm.tm_mon + 1, s_tm.tm_mday,
			s_tm.tm_hour, s_tm.tm_min, s_tm.tm_sec);
	printk("=============================\n");
	printk("cur_sys %d-%02d-%02d %02d:%02d:%02d.\r\n",
			s_tm.tm_year + 1900, s_tm.tm_mon + 1, s_tm.tm_mday,
			s_tm.tm_hour, s_tm.tm_min, s_tm.tm_sec);

	s2m_data_to_tm(r_data, &r_tm);
	sprintf(rtc_buf, "cur_rtc: %d-%02d-%02d %02d:%02d:%02d.\r\n",
			r_tm.tm_year + 1900, r_tm.tm_mon + 1, r_tm.tm_mday,
			r_tm.tm_hour, r_tm.tm_min, r_tm.tm_sec);
	printk("cur_rtc: %d-%02d-%02d %02d:%02d:%02d.\r\n",
			r_tm.tm_year + 1900, r_tm.tm_mon + 1, r_tm.tm_mday,
			r_tm.tm_hour, r_tm.tm_min, r_tm.tm_sec);

	rtc_tm_to_time(&r_tm, &r_time);


	if ((t_gap = abs(r_time - s_time.tv_sec)) > 60) {
		test_loop = 5;
	}

	if ((t_gap = abs(r_time - s_time.tv_sec)) > 2 || test_loop != 0) {
		if(test_loop !=0)
			test_loop--;

		pos = get_fs();
		set_fs(get_ds());
		fp = filp_open(RTC_GAP_DEBUG_LOGFILE, O_RDWR | O_APPEND | O_CREAT, 0644);
		if (IS_ERR(fp)) {
			printk("|RTC DEBUG | Failed to create log file |%s|, retry ...!\n", RTC_GAP_DEBUG_LOGFILE);
			fp = filp_open(RTC_GAP_DEBUG_LOGFILE, O_RDWR | O_APPEND | O_CREAT, 0644);
			if (IS_ERR(fp)) {
				printk("|RTC DEBUG | Failed to create log file |%s|, retry ...!\n", RTC_GAP_DEBUG_LOGFILE);
				set_fs(pos);
		...
		sprintf(rtc_log_buf, "|RTC CHECK | RTC time: %s, SYS time: %s. Error: %ld sec\n", rtc_buf, sys_buf, t_gap);
		err = fp->f_op->write(fp, rtc_log_buf, strlen(rtc_log_buf), &fp->f_pos);
		printk("%s", rtc_log_buf);
...
out:
	mutex_unlock(&g_rtc_info->lock);
	return ;
}

static void s2m_rtc_debug_worker(struct work_struct *data)
{
	if(stop_timer_logging == 1) return;	// skip race condition of resume and work-queue
	s2m_rtc_debug_function();
}

enum hrtimer_restart s2m_rtc_debug_timer_callback(struct hrtimer *timer)
{
	struct timespec ts;

	if(rtc_debug_wq != NULL) {
		queue_work(rtc_debug_wq , &rtc_debug_work);
		ts.tv_sec = 30;
		ts.tv_nsec = 0;
		hrtimer_start(&rtc_debug_hrtimer, timespec_to_ktime(ts), HRTIMER_MODE_REL);
	}
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	rtc_debug_wq = alloc_workqueue("rtc debug workqueue", 0, 1);
	if (rtc_debug_wq == NULL) {
		printk("@@@ BOBO_RTC @@@ failed to create workqueue for rtc.\r\n");
		goto tmp_out;
	}
	INIT_WORK(&rtc_debug_work, s2m_rtc_debug_worker);

	hrtimer_init(&rtc_debug_hrtimer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	rtc_debug_hrtimer.function = s2m_rtc_debug_timer_callback;
	ts.tv_sec = 30;
	ts.tv_nsec = 0;
	hrtimer_start(&rtc_debug_hrtimer, timespec_to_ktime(ts), HRTIMER_MODE_REL);
...
#ifdef CONFIG_STORE_RTC_TIMER_LOG
	hrtimer_cancel(&rtc_debug_hrtimer);
	if (rtc_debug_wq != NULL)
		destroy_workqueue(rtc_debug_wq);
#endif

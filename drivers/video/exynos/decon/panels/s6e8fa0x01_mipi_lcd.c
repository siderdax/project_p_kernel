...
void lcd_s6e8fa0x01_init(int id, struct decon_lcd *lcd)
{
	printk("lcd_init\n");
...
{
	lcd_s6e8fa0x01_init(dsim->id, &dsim->lcd_info);
	return 1;
}
...
static int s6e8fa0x01_dsim_probe(struct dsim_device *dsim)
{
	if (dsim->id == 0){
...
#ifdef CONFIG_SUPPORT_CODEGEN
static int s6e8fa0x01_probe(struct platform_device *pdev)
	{
	mipi_lcd_driver.probe		= s6e8fa0x01_dsim_probe;
	mipi_lcd_driver.displayon = s6e8fa0x01_displayon;
	mipi_lcd_driver.suspend	= s6e8fa0x01_suspend;
	mipi_lcd_driver.resume		= s6e8fa0x01_resume;

	return 0;
}

static int s6e8fa0x01_remove(struct platform_device *pdev)
{
	return 0;
}

static const struct of_device_id s6e8fa0x01_device_table[] = {
	        { .compatible = "samsung,s6e8fa0x01" },
		{},
};
static struct platform_driver s6e8fa0x01_driver __refdata = {
	.probe		= s6e8fa0x01_probe,
	.remove		= s6e8fa0x01_remove,
	.driver = {
		.name	= "exynos-mipi-dsi-s6e8fa0x01",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(s6e8fa0x01_device_table),
	}
};

static int exynos_s6e8fa0x01_register(void)
{
	platform_driver_register(&s6e8fa0x01_driver);

	return 0;
}

static void exynos_s6e8fa0x01_unregister(void)
{
	platform_driver_unregister(&s6e8fa0x01_driver);
}
module_init(exynos_s6e8fa0x01_register);
module_exit(exynos_s6e8fa0x01_unregister);

#else
struct mipi_dsim_lcd_driver mipi_lcd_driver= {
	.probe		= s6e8fa0x01_dsim_probe,

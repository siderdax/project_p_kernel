...
static void __decon_update_regs(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	for (i = 0; i < decon->pdata->max_win; i++) {
		...
		/* video mode: 0, dp: 1 mipi command mode: 2 */
//		if(decon->pdata->psr_mode == DECON_MIPI_COMMAND_MODE){ //remove after add DPU feature
			if (!is_vpp_type(regs->vpp_config[i].idma_type)) {
...
static int decon_dpu_cont_set(struct seq_file *s, void *unused)
{
...
static int decon_dpu_ad_set(struct seq_file *s, void *unused)
{
	return 0;
}
...
static ssize_t decon_dpu_cont_write(struct file *file, const char __user *buffer,
			size_t count, loff_t *ppos)
{
	int i, j, val, ret = 0;
	u32 offset, gamma, mask, length = 0;
	...
	case 0:
		for(j = 0; j < 3; j++) {
			for(i = 0; i < 65; i++) {
				...
				dpu_reg_set_contrast(offset, mask, gamma);
				mask = 0;
				gamma = 0;
			}
			offset = 0;
	...
	decon_int_drvdata->dpu_save.contrast_set = val;
...
static ssize_t decon_dpu_ad_write(struct file *file, const char __user *buffer,
			size_t count, loff_t *ppos)
{
	int val, ret = 0;
	u32 length = 0;
...
static int decon_dpu_cont_open(struct inode *inode, struct file *file)
{
	return single_open(file, decon_dpu_cont_set, inode->i_private);
}
...
static int decon_dpu_ad_open(struct inode *inode, struct file *file)
{
	return single_open(file, decon_dpu_ad_set, inode->i_private);
}
...
static struct file_operations decon_dpu_cont_ops = {
	.open = decon_dpu_cont_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
	.write   = decon_dpu_cont_write,
};
...
static struct file_operations decon_dpu_ad_ops = {
	.open = decon_dpu_ad_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
	.write   = decon_dpu_ad_write,
};
...
static int decon_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_EXYNOS_DECON_DPU)
	snprintf(debug_name, MAX_NAME_SIZE, "contrast");
	decon->dpu_set = debugfs_create_file(debug_name, 0444,
						decon->debug_root,
						decon, &decon_dpu_cont_ops);
	...
	snprintf(debug_name, MAX_NAME_SIZE, "ad");
	decon->dpu_set = debugfs_create_file(debug_name, 0444,
						decon->debug_root,
						decon, &decon_dpu_ad_ops);
...
#if 0	// move lcd mipi command ops to dsim function fei.xu@samsung.com		

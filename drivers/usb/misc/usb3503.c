...
static int usb3503_connect(struct usb3503 *hub)
{
	...
	if (hub->regmap) {
		...
		err = regmap_write(hub->regmap, USB3503_CFG1,reg_data);
		if (err < 0) {
		...
		reg_data = 0x00;
		err = regmap_write(hub->regmap, USB3503_OCS, reg_data);
		if (err < 0) {
			dev_err(dev, "OCS failed (%d)\n", err);
			return err;
		}
...
static int usb3503_probe(struct usb3503 *hub)
{
	...
	printk("%s=======================eric  hub->secondary_ref_clk =expected -> 0 , actual=(%d)\n",__func__,hub->secondary_ref_clk);

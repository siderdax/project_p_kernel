	...
	unsigned int input)
{
	...
	ret = fimc_is_ischain_isp_s_input(device, stream, module, vindex, memory, leader);
	if (ret) {
					...
					struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_IS_DVFS_UNLOCK:
		if (device->resourcemgr->dvfs_ctrl.cur_int_qos == DVFS_L0)
			i2c_clk = I2C_L0;
		else
			i2c_clk = I2C_L1;
		...
		mdbgv_isp("V4L2_CID_IS_DVFS_UNLOCK : %d I2C(%d)\n", vctx, ctrl->value, i2c_clk);
		break;
	default:
...
static int fimc_is_ixs_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vbq->drv_priv;
	struct fimc_is_queue *queue;
...
static int fimc_is_ixs_buffer_finish(struct vb2_buffer *vb)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vb->vb2_queue->drv_priv;
	struct fimc_is_device_ischain *device;

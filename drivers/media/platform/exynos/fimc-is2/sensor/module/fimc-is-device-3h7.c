...
static struct fimc_is_vci vci_3h7[] = {
	{
...
#ifdef CONFIG_OF
static int sensor_3h7_power_setpin(struct platform_device *pdev,
...
int sensor_3h7_probe(struct platform_device *pdev)
{
	int ret = 0;
...
#ifdef CONFIG_OF
	fimc_is_sensor_module_parse_dt(pdev, sensor_3h7_power_setpin);
#endif
	...
	module->pdata = pdata;
	module->pdev = pdev;
	module->sensor_id = SENSOR_NAME_S5K3H7;
	...
	module->vcis = ARRAY_SIZE(vci_3h7);
	module->vci = vci_3h7;
	...
	ext->sensor_con.product_name = SENSOR_NAME_S5K3H7;
...
static int sensor_3h7_remove(struct platform_device *pdev)
{
	int ret = 0;
...
#ifdef CONFIG_OF
static const struct of_device_id exynos_fimc_is_sensor_3h7_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-sensor-3h7",
	},
	{},
};
#endif
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_3h7_match);

static struct platform_driver sensor_3h7_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = exynos_fimc_is_sensor_3h7_match
#endif
	},
	.probe	= sensor_3h7_probe,
	.remove	= sensor_3h7_remove,
};

module_platform_driver(sensor_3h7_driver);

static int __init fimc_is_sensor_module_init(void)
{
	int ret = 0;

	ret = platform_driver_register(&sensor_3h7_driver);
	if (ret)
...
{
	platform_driver_unregister(&sensor_3h7_driver);
}

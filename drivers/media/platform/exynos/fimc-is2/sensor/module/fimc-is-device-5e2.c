...
static struct fimc_is_sensor_cfg config_5e2[] = {
	...
	FIMC_IS_SENSOR_CFG(2576, 1930, 30, 19, 0),
	FIMC_IS_SENSOR_CFG(2576, 1708, 30, 19, 0),
	/* 1936x1090@30fps */
	FIMC_IS_SENSOR_CFG(2576, 1452, 30, 19, 1),
	/* 1936x1090@24fps */
	FIMC_IS_SENSOR_CFG(1936, 1092, 24, 19, 2),
	/* 1296x730@30fps */
	FIMC_IS_SENSOR_CFG(1296, 730, 30, 19, 2),
};
...
static int sensor_5e2_open(struct v4l2_subdev *sd,
	struct v4l2_subdev_fh *fh)
{
	pr_info("%s\n", __func__);
	return 0;
}
static int sensor_5e2_close(struct v4l2_subdev *sd,
	struct v4l2_subdev_fh *fh)
{
	pr_info("%s\n", __func__);
	return 0;
}
static int sensor_5e2_registered(struct v4l2_subdev *sd)
{
	pr_info("%s\n", __func__);
	return 0;
}

static void sensor_5e2_unregistered(struct v4l2_subdev *sd)
{
	pr_info("%s\n", __func__);
}

static const struct v4l2_subdev_internal_ops internal_ops = {
	.open = sensor_5e2_open,
	.close = sensor_5e2_close,
	.registered = sensor_5e2_registered,
	.unregistered = sensor_5e2_unregistered,
};
...
static int sensor_5e2_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	struct fimc_is_module_5e2 *module_5e2;
	...
	module_5e2 = module->private_data;
	if (unlikely(!module_5e2)) {
		err("module_5e2 is NULL");
		ret = -EINVAL;
		goto p_err;
	}
	...
	module_5e2->system_clock = 146 * 1000 * 1000;
	module_5e2->line_length_pck = 146 * 1000 * 1000;
...
static int sensor_5e2_s_stream(struct v4l2_subdev *subdev, int enable)
{
	int ret = 0;
...
static int sensor_5e2_s_param(struct v4l2_subdev *subdev, struct v4l2_streamparm *param)
{
	int ret = 0;
	struct fimc_is_module_enum *sensor;
...
static int sensor_5e2_s_format(struct v4l2_subdev *subdev, struct v4l2_mbus_framefmt *fmt)
{
	int ret = 0;
	struct fimc_is_module_enum *module;
	struct fimc_is_module_5e2 *module_5e2;
...
static const struct v4l2_subdev_video_ops video_ops = {
	.s_stream = sensor_5e2_s_stream,
	.s_parm = sensor_5e2_s_param,
	.s_mbus_fmt = sensor_5e2_s_format
};
...
int sensor_5e2_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_module_enum *sensor;
	struct i2c_client *client;
...
int sensor_5e2_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_module_enum *sensor;
	struct i2c_client *client;
 ...
 * @ remarks
 */
int sensor_5e2_s_duration(struct v4l2_subdev *subdev, u64 duration)
{
	int ret = 0;
	u8 value[2];
...
int sensor_5e2_g_min_duration(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_g_max_duration(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_s_exposure(struct v4l2_subdev *subdev, u64 exposure)
{
	int ret = 0;
	u8 value;
	struct fimc_is_module_enum *sensor;
...
int sensor_5e2_g_min_exposure(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_g_max_exposure(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_s_again(struct v4l2_subdev *subdev, u64 sensitivity)
{
	int ret = 0;
...
int sensor_5e2_g_min_again(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_g_max_again(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_s_dgain(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_g_min_dgain(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}

int sensor_5e2_g_max_dgain(struct v4l2_subdev *subdev)
{
	int ret = 0;
	return ret;
}
		...
		struct exynos_platform_fimc_is_module *pdata)
{
	...
	gpio_vdda = of_get_named_gpio(dnode, "gpio_vdda", 0);
	gpio_vddd = of_get_named_gpio(dnode, "gpio_vddd", 0);
      gipo_vddio = of_get_named_gpio(dnode, "gpio_vddio", 0);
...
//#ifdef CONFIG_MACH_ESPRESSO7420
	...
	//SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "VDD18_VTCAMIO", PIN_REGULATOR, 1, 0);
       ...
       SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_vdda, "gpio_vdda", PIN_OUTPUT, 1, 0);
       SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gipo_vddio, "gpio_vddio", PIN_OUTPUT, 1, 0);
	 SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_vddd, "gpio_vddd", PIN_OUTPUT, 1, 0);
	...
	//SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_standby, "standby low", PIN_OUTPUT, 1, 0);
...
printk("lmid %s(%d)\n", __func__, ret);
	...
	module->lanes = CSI_DATA_LANES_2;// by lmd
	...
	module->private_data = kzalloc(sizeof(struct fimc_is_module_5e2), GFP_KERNEL);
...
module_platform_driver(sensor_5e2_driver);

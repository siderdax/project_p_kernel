...
static int sensor_2p2_remove(struct platform_device *pdev)
{
	int ret = 0;
...
static const struct of_device_id exynos_fimc_is_sensor_2p2_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-sensor-2p2",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_2p2_match);

static struct platform_driver sensor_2p2_driver = {
	.probe  = sensor_2p2_probe,
	.remove = sensor_2p2_remove,
	.driver = {
		.name   = SENSOR_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = exynos_fimc_is_sensor_2p2_match,
	}
};

module_platform_driver(sensor_2p2_driver);

static int __init fimc_is_sensor_module_init(void)
{
	int ret = 0;

	ret = platform_driver_register(&sensor_2p2_driver);
	if (ret)
...
{
	platform_driver_unregister(&sensor_2p2_driver);
}

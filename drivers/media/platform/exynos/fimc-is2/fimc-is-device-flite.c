...
static int flite_hw_set_bns(u32 __iomem *base_reg,
...
static void flite_hw_set_cam_channel(u32 __iomem *base_reg,
	u32 otf_setting)
{
	u32 cfg;
...
static void flite_hw_set_output_dma(u32 __iomem *base_reg, bool enable)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static bool flite_hw_get_output_dma(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_set_output_local(u32 __iomem *base_reg, bool enable)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_set_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_clr_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_set_ovf_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_clr_ovf_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static int flite_hw_check_ovf_interrupt_source(u32 __iomem *base_reg)
{
	u32 cfg = 0;
	cfg = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIGCTRL));
...
static void flite_hw_set_inverse_polarity(u32 __iomem *base_reg)
{
	u32 cfg = 0;
...
int flite_hw_get_present_frame_buffer(u32 __iomem *base_reg)
{
	u32 status = 0;
...
int flite_hw_get_status2(u32 __iomem *base_reg)
{
	u32 status = 0;
...
void flite_hw_set_status1(u32 __iomem *base_reg, u32 val)
{
	writel(val, base_reg + TO_WORD_OFFSET(FLITE_REG_CISTATUS));
}

int flite_hw_get_status1(u32 __iomem *base_reg)
{
	u32 status = 0;
...
int flite_hw_getnclr_status1(u32 __iomem *base_reg)
{
	u32 status = 0;

	status = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CISTATUS));
	writel(status, base_reg + TO_WORD_OFFSET(FLITE_REG_CISTATUS));

	return status;
}

void flite_hw_set_status2(u32 __iomem *base_reg, u32 val)
{
	writel(val, base_reg + TO_WORD_OFFSET(FLITE_REG_CISTATUS2));
}

void flite_hw_set_start_addr(u32 __iomem *base_reg, u32 number, u32 addr)
{
...
void flite_hw_set_use_buffer(u32 __iomem *base_reg, u32 number)
{
	u32 buffer;
...
void flite_hw_set_unuse_buffer(u32 __iomem *base_reg, u32 number)
{
	u32 buffer;
	buffer = readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIFCNTSEQ));
	buffer &= ~(1<<number);
...
u32 flite_hw_get_buffer_seq(u32 __iomem *base_reg)
{
	return readl(base_reg + TO_WORD_OFFSET(FLITE_REG_CIFCNTSEQ));
}

void flite_hw_set_mux(u32 __iomem *base_reg, u32 csi_ch, u32 flite_ch)
{
	u32 cfg;
...
void flite_hw_set_mux_taa_isp(u32 __iomem *base_reg, u32 taa_ch, u32 isp_ch)
{
	u32 cfg;

	if (((taa_ch == 0) && (isp_ch == 0)) || ((taa_ch == 1) && (isp_ch == 1))) {
		/*
		 * OTF CONFIG
		 * 3AA0 -> ISP0 OTF
		 * 3AA1 -> ISP1 OTF
...
int init_fimc_lite(u32 __iomem *base_reg)
{
	int i;
	...
	struct fimc_is_device_ischain *ischain)
{
	...
	if (ischain && !test_bit(FIMC_IS_ISCHAIN_REPROCESSING, &ischain->state)) {
		/* set mux setting for otf between 3aa and isp */
		flite_hw_set_mux_taa_isp(base_reg,
			ischain->group_3aa.id - GROUP_ID_3AA0,
			ischain->group_isp.id - GROUP_ID_ISP0);
	}
#endif
...
static inline void stop_fimc_lite(u32 __iomem *base_reg)
{
	...
	u32 bindex)
{
	...
	if (!atomic_read(&flite->bcount)) {
		...
		flite_hw_set_output_dma(flite->base_reg, true);
	} else {
		flite_hw_set_use_buffer(flite->base_reg, bindex);
		atomic_inc(&flite->bcount);
	}
	...
	u32 bindex)
{
	...
	if (atomic_read(&flite->bcount) == 1) {
		...
		flite_hw_set_output_dma(flite->base_reg, false);
...
void tasklet_flite_str0(unsigned long data)
{
	...
	if (unlikely(list_empty(&group_3aa->smp_trigger.wait_list))) {
		...
		if (((g_print_cnt % LOG_INTERVAL_OF_DROPS) == 0) ||
			...
			info("grp1(res %d, rcnt %d, scnt %d), "
				...
				groupmgr->group_smp_res[group_3aa->slot].count,
...
void tasklet_flite_str1(unsigned long data)
{
	struct v4l2_subdev *subdev;
	struct fimc_is_device_flite *flite;
	u32 bstart, fcount, present;
...
void tasklet_flite_str2(unsigned long data)
{
        struct v4l2_subdev *subdev;
        struct fimc_is_device_flite *flite;
        struct fimc_is_device_sensor *device;
...
void tasklet_flite_str3(unsigned long data)
{
        struct v4l2_subdev *subdev;
        struct fimc_is_device_flite *flite;
        struct fimc_is_framemgr *framemgr;
...
static void tasklet_flite_end0(unsigned long data)
{
	struct fimc_is_device_flite *flite;
	struct fimc_is_framemgr *framemgr;
	struct fimc_is_frame *frame;
	...
	framemgr_e_barrier(framemgr, FMGR_IDX_11);
	...
	framemgr_x_barrier(framemgr, FMGR_IDX_11);
...
static void tasklet_flite_end0_chk(unsigned long data)
{
...
static void tasklet_flite_end1(unsigned long data)
{
        struct fimc_is_device_flite *flite;
        struct fimc_is_framemgr *framemgr;
        struct fimc_is_frame *frame;
...
irqreturn_t fimc_is_flite_isr_1slot(int irq, void *data)
{
...
irqreturn_t fimc_is_flite_isr_2slot(int irq, void *data)
{
	u32 status, status1, status2, i;
	struct fimc_is_device_flite *flite;
...
#ifdef ENABLE_ONE_SLOT
			fimc_is_flite_isr_1slot,
#else
			fimc_is_flite_isr_2slot,
#endif
			IRQF_SHARED,
			"fimc-lite0",
			flite);
	...
	struct fimc_is_device_flite *flite)
{
...
#ifdef ENABLE_ONE_SLOT
	flite_hw_set_use_buffer(flite->base_reg, FLITE_A_SLOT_VALID);
	...
	if (device->ischain &&
...
#ifdef ENABLE_ONE_SLOT
		tasklet_init(&flite->tasklet_flite_str, tasklet_flite_str2, (unsigned long)subdev);
		tasklet_init(&flite->tasklet_flite_end, tasklet_flite_end1, (unsigned long)subdev);
#else
		tasklet_init(&flite->tasklet_flite_str, tasklet_flite_str0, (unsigned long)subdev);
		tasklet_init(&flite->tasklet_flite_end, tasklet_flite_end0, (unsigned long)subdev);
#endif
		...
		if (flite->instance == FLITE_ID_A) {
			if (flite->group == GROUP_ID_3AA0)
				otf_setting = FLITE_REG_CIGENERAL_3AA0_CAM_A;
			else
				otf_setting = FLITE_REG_CIGENERAL_3AA1_CAM_A;
		} else if (flite->instance == FLITE_ID_B) {
			if (flite->group == GROUP_ID_3AA0)
				otf_setting = FLITE_REG_CIGENERAL_3AA0_CAM_B;
			else
				otf_setting = FLITE_REG_CIGENERAL_3AA1_CAM_B;
		} else if (flite->instance == FLITE_ID_C) {
			if (flite->group == GROUP_ID_3AA0)
				otf_setting = FLITE_REG_CIGENERAL_3AA0_CAM_C;
			else
				otf_setting = FLITE_REG_CIGENERAL_3AA1_CAM_C;
		} else {
...
#ifdef ENABLE_ONE_SLOT
			tasklet_init(&flite->tasklet_flite_str, tasklet_flite_str3, (unsigned long)subdev);
			tasklet_init(&flite->tasklet_flite_end, tasklet_flite_end1, (unsigned long)subdev);
#else
		...
		case FLITE_BUF_DONE_EARLY:
			...
			flite->early_work_called = false;
			tasklet_init(&flite->tasklet_flite_str, tasklet_flite_str1, (unsigned long)subdev);
			tasklet_init(&flite->tasklet_flite_early_end, tasklet_flite_end0, (unsigned long)subdev);
			tasklet_init(&flite->tasklet_flite_end, tasklet_flite_end0_chk, (unsigned long)subdev);
			break;
		default:
			tasklet_init(&flite->tasklet_flite_str, tasklet_flite_str1, (unsigned long)subdev);
			tasklet_init(&flite->tasklet_flite_end, tasklet_flite_end0, (unsigned long)subdev);
	...
	u32 instance)
{
	...
	switch(instance) {
	case FLITE_ID_A:
		flite->base_reg = (u32 *)FIMCLITE0_REG_BASE;
		break;
	case FLITE_ID_B:
		flite->base_reg = (u32 *)FIMCLITE1_REG_BASE;
		break;
	case FLITE_ID_C:
		flite->base_reg = (u32 *)FIMCLITE2_REG_BASE;
		break;
	case FLITE_ID_D:
		flite->base_reg = (u32 *)FIMCLITE3_REG_BASE;
		break;
	default:

...
{
	unsigned int hibern8_enter_delay = hba->clk_gating.delay_ms;
	...
	if (hba->vops && hba->vops->ufshcd_get_hibern8_enter_delay)
		hibern8_enter_delay =
			hba->vops->ufshcd_get_hibern8_enter_delay(hba);

	if (hba->clk_gating.delay_ms > hibern8_enter_delay)
		hibern8_enter_delay = hba->clk_gating.delay_ms;

...
#define TP_MON_PERIOD	(HZ / 10)

/* PM QoS Expiration Time in TP mon */
#define TP_MON_PM_QOS_LIFETIME	(200000)
...
static void exynos_ufs_pm_qos_add(struct exynos_ufs *ufs)
{
	...
	schedule_delayed_work_on(0x0, &ufs->tp_mon, TP_MON_PERIOD);
}
...
static void exynos_ufs_tp_mon(struct work_struct *work)
{
	...
	if (tp_tbl->threshold) {
		...
		hibern8_enter_delay = HIBERN8_ENTER_DELAY_DEF * 4;
	} else {
	...
	if (atomic_read(&ufs->tpmon_log)) {
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
		dev_info(ufs->dev, "%d bytes/s mif=%d big=%d little=%d\n",
						hba->tp_per_period,
						tp_tbl->mif_value,
						tp_tbl->cluster1_value,
						tp_tbl->cluster0_value);
#else
		dev_info(ufs->dev, "%d bytes/s mif=%d big=%d\n",
						hba->tp_per_perfiod,
						tp_tbl->mif_value,
						tp_tbl->cluster1_value);
#endif
	...
	if (atomic_read(&ufs->tpmon_enable))
		schedule_delayed_work_on(0x0, &ufs->tp_mon, TP_MON_PERIOD);
}
...
static ssize_t tpmon_log_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct exynos_ufs *ufs = dev->platform_data;
	return snprintf(buf, PAGE_SIZE, "%u\n", atomic_read(&ufs->tpmon_log));
}

static ssize_t tpmon_log_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	atomic_set(&ufs->tpmon_log, value);
	return count;
}

static ssize_t tpmon_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct exynos_ufs *ufs = dev->platform_data;
	return snprintf(buf, PAGE_SIZE, "%u\n", atomic_read(&ufs->tpmon_enable));
}

static ssize_t tpmon_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct exynos_ufs *ufs = dev->platform_data;
	unsigned long value;
	...
	atomic_set(&ufs->tpmon_enable, value);
	if (atomic_read(&ufs->tpmon_enable))
...
static inline void exynos_ufs_tpmon_sysfs_init(struct exynos_ufs *ufs)
{
	ufs->tpmon_log_attr.show = tpmon_log_show;
	ufs->tpmon_log_attr.store = tpmon_log_store;
	sysfs_attr_init(&ufs->tpmon_log_attr.attr);
	ufs->tpmon_log_attr.attr.name = "tpmon_log";
	ufs->tpmon_log_attr.attr.mode = S_IRUGO | S_IWUSR | S_IWGRP;
	if (device_create_file(ufs->dev, &ufs->tpmon_log_attr))
		dev_err(ufs->dev, "Failed to create tpmon_log sysfs entry\n");

	ufs->tpmon_enable_attr.show = tpmon_enable_show;
	ufs->tpmon_enable_attr.store = tpmon_enable_store;
	sysfs_attr_init(&ufs->tpmon_enable_attr.attr);
	ufs->tpmon_enable_attr.attr.name = "tpmon_enable";
	ufs->tpmon_enable_attr.attr.mode = S_IRUGO | S_IWUSR | S_IWGRP;
	if (device_create_file(ufs->dev, &ufs->tpmon_enable_attr))
		dev_err(ufs->dev, "Failed to create tpmon_enable sysfs entry\n");
}
...
static int exynos_ufs_probe(struct platform_device *pdev)
{
	...
	INIT_DEFERRABLE_WORK(&ufs->tp_mon, exynos_ufs_tp_mon);
	exynos_ufs_pm_qos_add(ufs);

	atomic_set(&ufs->tpmon_log, 0);
	atomic_set(&ufs->tpmon_enable, 1);
	exynos_ufs_tpmon_sysfs_init(ufs);

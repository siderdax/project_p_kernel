...
#ifdef CONFIG_EXYNOS_ISP_DVFS_RANDOM_TEST
static unsigned int exynos_random_isp_freq_get(struct device *dev,struct devfreq *devfreq_isp);
#endif
...
#ifdef CONFIG_EXYNOS_ISP_DVFS_RANDOM_TEST
	*target_freq = exynos_random_isp_freq_get(dev,devfreq_isp);
#endif
...
static unsigned int exynos_random_isp_freq_get(struct device *dev,struct devfreq *devfreq_isp)
{
	...
	freq = devfreq_isp->min_freq;
	min_support_idx =  devfreq_get_opp_idx(devfreq_isp_opp_list,
							data->max_state,
							freq);

	freq = devfreq_isp->max_freq;
	max_support_idx =  devfreq_get_opp_idx(devfreq_isp_opp_list,
							data->max_state,
							freq);
	...
	return exynos7_devfreq_isp_profile.freq_table[idx + max_support_idx];
}
#endif

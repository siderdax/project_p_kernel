...
#ifdef CONFIG_EXYNOS_DISP_DVFS_RANDOM_TEST
static unsigned int exynos_random_disp_freq_get(struct device *dev,struct devfreq *devfreq_disp);
#endif
...
#ifdef CONFIG_EXYNOS_DISP_DVFS_RANDOM_TEST
	*target_freq = exynos_random_disp_freq_get(dev, devfreq_disp);
#endif
...
static unsigned int exynos_random_disp_freq_get(struct device *dev,struct devfreq *devfreq_disp)
{
	...
	freq = devfreq_disp->min_freq;
	min_support_idx =  devfreq_get_opp_idx(devfreq_disp_opp_list,
							data->max_state,
							freq);

	freq = devfreq_disp->max_freq;
	max_support_idx =  devfreq_get_opp_idx(devfreq_disp_opp_list,
							data->max_state,
							freq);
	...
	return exynos7_devfreq_disp_profile.freq_table[idx + max_support_idx];
}
#endif

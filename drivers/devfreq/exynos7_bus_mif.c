...
#ifdef CONFIG_EXYNOS_MIF_DVFS_RANDOM_TEST
static unsigned int exynos_random_mif_freq_get(struct device *dev,struct devfreq *devfreq_mif);
#endif
...
#ifdef CONFIG_EXYNOS_MIF_DVFS_RANDOM_TEST
	struct devfreq_simple_exynos_data *Governor_simple_exynos_data = devfreq_mif->data;
...
#ifdef CONFIG_EXYNOS_MIF_DVFS_RANDOM_TEST
	*target_freq = exynos_random_mif_freq_get(dev,devfreq_mif);

	if (Governor_simple_exynos_data) {
		pm_qos_min = pm_qos_request(Governor_simple_exynos_data->pm_qos_class);
		if (Governor_simple_exynos_data->pm_qos_class_max)
			pm_qos_max = pm_qos_request(Governor_simple_exynos_data->pm_qos_class_max);

		if (Governor_simple_exynos_data->cal_qos_max) {
			cal_qos_max = Governor_simple_exynos_data->cal_qos_max;
		} else {
			cal_qos_max = devfreq_mif->max_freq;
		}
	}

	if (*target_freq > cal_qos_max)
		*target_freq = cal_qos_max;

	if (pm_qos_min)
		*target_freq = max(pm_qos_min, *target_freq);

	if (pm_qos_max)
		*target_freq = min(pm_qos_max, *target_freq);
#endif
...
static unsigned int exynos_random_mif_freq_get(struct device *dev,struct devfreq *devfreq_mif)
{
	...
	freq = devfreq_mif->min_freq;
	min_support_idx =  devfreq_get_opp_idx(devfreq_mif_opp_list,
							mif_data->max_state,
							freq);

	freq = devfreq_mif->max_freq;
	max_support_idx =  devfreq_get_opp_idx(devfreq_mif_opp_list,
							mif_data->max_state,
							freq);
	...
	return exynos7_devfreq_mif_profile.freq_table[idx + max_support_idx];
}
#endif

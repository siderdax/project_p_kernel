...
#define DEVFREQ_INITIAL_FREQ	(560000)
...
#ifdef CONFIG_EXYNOS_INT_DVFS_RANDOM_TEST
static unsigned int exynos_random_int_freq_get(struct device *dev,struct devfreq *devfreq_int);
#endif
...
#ifdef CONFIG_EXYNOS_INT_DVFS_RANDOM_TEST
	*target_freq = exynos_random_int_freq_get(dev,devfreq_int);
#endif
...
static unsigned int exynos_random_int_freq_get(struct device *dev,struct devfreq *devfreq_int)
{
	...
	freq = devfreq_int->min_freq;
	min_support_idx =  devfreq_get_opp_idx(devfreq_int_opp_list,
							data->max_state,
							freq);

	freq = devfreq_int->max_freq;
	max_support_idx =  devfreq_get_opp_idx(devfreq_int_opp_list,
							data->max_state,
							freq);
	...
	return exynos7_devfreq_int_profile.freq_table[idx + max_support_idx];
}
#endif

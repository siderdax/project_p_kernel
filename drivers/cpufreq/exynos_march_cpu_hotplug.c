...
#if defined(CONFIG_SCHED_HMP)
int change_core_num_cluster0(int target_num, int use_target)
{
	int ret = 0;
	int cur_num, val, i = 0;
...
int change_core_num_cluster1(int cluster_on_off, int hmp_booster)
{
	int ret = 0;
	int cur_num, val, i = 0;
...
static int calc_load_cluster(int cluster_num, unsigned int *load)
{
	...
	for_each_cpu_and(i, cpu_coregroup_mask(cluster_num*(NR_CLUST1_CPUS-1)), cpu_online_mask) {
		int ret = 0;
		unsigned long flags;
		spin_lock_irqsave(&per_cpu(cpuload, i).cpu_load_lock, flags);
...
static int __init march_cpu_hotplug_init(void)
{
	...
	pm_qos_update_request_timeout(&cluster1_num_boot_min_qos, NR_CLUST1_CPUS, 25 * USEC_PER_SEC);
	pm_qos_update_request_timeout(&cluster0_num_boot_min_qos, NR_CLUST0_CPUS, 25 * USEC_PER_SEC);

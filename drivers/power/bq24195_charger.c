...
#define bq24195_DBG
static int bq24195_dbg = 0;
#ifdef bq24195_DBG
//#define bq24195_print(fmt, args...) printk(fmt, ##args)
#define bq24195_print(fmt, ...) do { if (bq24195_dbg) printk("bq24195: "fmt, ##__VA_ARGS__); } while(0)
#else
//#define bq24195_print(fmt, args...)
#define bq24195_print(fmt, ...)
#endif
...
#define DPDM_EN_MASK			(1<<DPDM_EN_SHIFT)
...
static struct bq24195_chip *this_chip = NULL;

static int bq24195_read_reg(int reg)
{
...
static int bq24195_write_reg(int reg, int value)
{
	struct i2c_client *client = this_chip->client;
	int ret;
...
static int bq24195_masked_write(int reg, u8 mask, u8 val)
{
	struct i2c_client *client = this_chip->client;
	int rc;
	int temp;

	rc = bq24195_read_reg(reg);
	if (rc < 0)
	{
...
static enum power_supply_property bq24195_power_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
};

static int bq24195_power_get_property(struct power_supply *ps, 
		...
		union power_supply_propval *val)
{
	...
	bq24195_print("psp:%d, type:%d, value:%d\n", psp, ps->type, val->intval);

	return 0;
}

static ssize_t bq24195_show_reg(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= 0x0a; i++)
	{
		ret = bq24195_read_reg(i);
		printk("%s, reg[%d]:0x%x\n", __FUNCTION__, i, ret);
	}
	return sprintf(buf, "\n");
}

static DEVICE_ATTR(reg, S_IRUGO, bq24195_show_reg, NULL);

static ssize_t bq24195_show_dbg(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", bq24195_dbg);
}

static ssize_t bq24195_store_dbg(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(debug, S_IRUGO|S_IWUSR, bq24195_show_dbg, bq24195_store_dbg);

static struct attribute *bq24195_attributes[] = {
	&dev_attr_reg.attr,
	&dev_attr_debug.attr,
	NULL
};

static const struct attribute_group bq24195_attr_group = {
	.attrs = bq24195_attributes,
};

static int bq24195_read_version(void)
{
	int ret;
	ret = bq24195_read_reg(VENDOR_PART_REV_STATUS_REG);
	bq24195_print("bq24195 Ver 0x%x\n", ret);
	return ret;
}
...
static int bq24195_set_input_vin_limit(int mv)
{
	struct i2c_client *client = this_chip->client;
	u8 reg_val = 0;
	int set_vin = 0;
	...
	bq24195_print("req_vin = %d set_vin = %d reg_val = 0x%02x\n",
				mv, set_vin, reg_val);

	return bq24195_masked_write(INPUT_SRC_CTRL_REG, INPUT_VOLTAGE_LIMIT_MASK, reg_val);
}

#define SYSTEM_VMIN_LOW_MV  3000
#define SYSTEM_VMIN_HIGH_MV  3700
#define SYSTEM_VMIN_STEP_MV  100
static int bq24195_set_system_vmin(int mv)
{
	struct i2c_client *client = this_chip->client;
	u8 reg_val = 0;
	int set_vmin = 0;
	...
	bq24195_print("req_vmin = %d set_vmin = %d reg_val = 0x%02x\n",
				mv, set_vmin, reg_val);

	return bq24195_masked_write(PWR_ON_CONF_REG, SYSTEM_MIN_VOLTAGE_MASK, reg_val);
}

#define IPRECHG_MIN_MA  128
#define IPRECHG_MAX_MA  2048
#define IPRECHG_STEP_MA  128
static int bq24195_set_prechg_i_limit(int ma)
{
	struct i2c_client *client = this_chip->client;
	u8 reg_val = 0;
	int set_ma = 0;
	...
	bq24195_print("req_i = %d set_i = %d reg_val = 0x%02x\n",
				ma, set_ma, reg_val);

	return bq24195_masked_write(PRE_CHARGE_TERM_CUR_CTRL_REG, PRECHG_CURRENT_MASK, reg_val);
}

#define ITERM_MIN_MA  128
#define ITERM_MAX_MA  2048
#define ITERM_STEP_MA  128
static int bq24195_set_term_current(int ma)
{
	struct i2c_client *client = this_chip->client;
	u8 reg_val = 0;
	int set_ma = 0;
	...
	return bq24195_masked_write(PRE_CHARGE_TERM_CUR_CTRL_REG, TERM_CURRENT_MASK, reg_val);
}

#define VBAT_MAX_MV  4400
#define VBAT_MIN_MV  3504
#define VBAT_STEP_MV  16
static int bq24195_set_vbat_max(int mv)
{
	struct i2c_client *client = this_chip->client;
	u8 reg_val = 0;
	int set_vbat = 0;
	...
	bq24195_print("req_vbat = %d set_vbat = %d reg_val = 0x%02x\n",
				mv, set_vbat, reg_val);

	return bq24195_masked_write(CHARGE_VOLT_CTRL_REG, CHG_VOLTAGE_LIMIT_MASK, reg_val);
}

static int bq24195_hw_init(void)
{
	struct i2c_client *client = this_chip->client;
	int ret;
	ret = bq24195_write_reg(PWR_ON_CONF_REG, RESET_REGISTER_MASK);
	if (ret < 0)
	{
		dev_err(&client->dev, "err hw init\n");
		return ret;
	}
	bq24195_set_input_vin_limit(4200);
	bq24195_set_system_vmin(3500);
	bq24195_set_prechg_i_limit(256);
	bq24195_set_term_current(128);
	bq24195_set_vbat_max(4200);
	bq24195_write_reg(CHARGE_TERM_TIMER_CTRL_REG, EN_CHG_TERM_MASK);
	bq24195_masked_write(PWR_ON_CONF_REG, BOOST_LIM_MASK, 0);
	bq24195_masked_write(MISC_OPERATION_CTRL_REG, DPDM_EN_MASK, 1<<DPDM_EN_SHIFT);
	return 0;
}
...
static int bq24195_set_ichg_current(int ma)
{
	struct i2c_client *client = this_chip->client;
	u8 reg_val = 0;
	int set_ichg = 0;
	...
	bq24195_print("req_ichg = %d set_ichg = %d reg_val = 0x%02x\n",
				ma, set_ichg, reg_val);

	return bq24195_masked_write(CHARGE_CUR_CTRL_REG, ICHG_MASK, reg_val);
}
static void bq24195_set_ac_charge_current(void)
{
	bq24195_set_ichg_current(1024);
}

static void bq24195_set_usb_charge_current(void)
{
	bq24195_set_ichg_current(512);
}

static void bq24195_status_func(struct work_struct *work)
{
	int ret;

	ret = bq24195_read_reg(SYSTEM_STATUS_REG);
	...
	if (this_chip->ac_online)
		bq24195_set_ac_charge_current();
	else if (this_chip->usb_online)
		bq24195_set_usb_charge_current();
	else
		bq24195_set_usb_charge_current();
...
static int bq24195_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct bq24195_chip *chip;
	...
	of_property_read_u32(np, "debug", &bq24195_dbg);
	...
	bq24195_read_version();

	chip->usb.name		= "usb";
	chip->usb.type		= POWER_SUPPLY_TYPE_USB;
	chip->usb.supplied_to	= supply_to_list;
	chip->usb.num_supplicants = ARRAY_SIZE(supply_to_list);
	chip->usb.get_property	= bq24195_power_get_property;
	chip->usb.properties	= bq24195_power_props;
	chip->usb.num_properties	= ARRAY_SIZE(bq24195_power_props);

	chip->ac.name		= "ac";
	chip->ac.type		= POWER_SUPPLY_TYPE_MAINS;
	chip->ac.supplied_to	= supply_to_list;
	chip->ac.num_supplicants = ARRAY_SIZE(supply_to_list);
	chip->ac.get_property	= bq24195_power_get_property;
	chip->ac.properties	= bq24195_power_props;
	chip->ac.num_properties	= ARRAY_SIZE(bq24195_power_props);
	...
	ret = sysfs_create_group(&client->dev.kobj, &bq24195_attr_group);
	if (ret)
	{
	...
	INIT_DELAYED_WORK(&chip->work_status, bq24195_status_func);

	chip->chg_int_gpio = of_get_named_gpio(np, "bq24195,chg_int", 0);
	...
	enable_irq_wake(chip->chg_int_irq);

	return 0;

err_request_irq:
err_pinctrl:
	sysfs_remove_group(&client->dev.kobj, &bq24195_attr_group);
err_create_sysfs:
...
static int bq24195_suspend(struct i2c_client *client,
		pm_message_t state)
{
	bq24195_print("%s()\n", __FUNCTION__);
	return 0;
...
static int bq24195_resume(struct i2c_client *client)
{
	bq24195_print("%s()\n", __FUNCTION__);
	return 0;
}
...
#define bq24195_suspend NULL
#define bq24195_resume NULL

#endif /* CONFIG_PM */

#ifdef CONFIG_OF
static struct of_device_id bq24195_of_match_table[] = {
	{
		.compatible = "ti,bq24195-chg",
	},
	{},
};
MODULE_DEVICE_TABLE(of, bq24195_of_match_table);
#endif

#define bq24195_I2C_NAME "bq24195_i2c"// TODO
...
static struct i2c_driver bq24195_i2c_driver = {
	.driver	= {
		.name	= bq24195_I2C_NAME,
		.owner	= THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = bq24195_of_match_table,
#endif
	},
	.probe		= bq24195_probe,
	.remove		= bq24195_remove,
	.suspend	= bq24195_suspend,
	.resume		= bq24195_resume,
	.id_table	= bq24195_id,
};

static int __init bq24195_init(void)
{
	
	printk("bq24195 charger driver: initialize\n");
	return i2c_add_driver(&bq24195_i2c_driver);
}
module_init(bq24195_init);

static void __exit bq24195_exit(void)
{
	i2c_del_driver(&bq24195_i2c_driver);
}
module_exit(bq24195_exit);

MODULE_AUTHOR("Eric Seo <eric.seo@mvlab.co.kr>");
MODULE_DESCRIPTION("bq24195 charger");
MODULE_LICENSE("GPL");

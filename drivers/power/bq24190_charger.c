...
#include <linux/i2c.h>
#if defined(CONFIG_MACH_ESPRESSO7420) && defined(CONFIG_FUELGAUGE_S2MG001)
...
static int bq24195_charging_status;

int bq24195_get_charging_status(void)
{
	return bq24195_charging_status;
}
EXPORT_SYMBOL_GPL(bq24195_get_charging_status);
		...
		union power_supply_propval *val)
{
	...
	bq24195_charging_status=status;
...
out:
	...
	dev_info(bdi->dev, "ss_reg: 0x%02x, f_reg: 0x%02x\n", ss_reg, f_reg);
		...
		const struct i2c_device_id *id)
{
	...
	device_init_wakeup(bdi->dev, 1);

	ret = enable_irq_wake(bdi->irq);
	if (ret) {
		dev_err(bdi->dev, "Failed to enable wakeup on IRQ request\n");
		goto out1;
	} else
		bdi->irq_wake_enabled = 1;
...
static int bq24190_remove(struct i2c_client *client)
{
	...
	if (bdi->irq_wake_enabled)
		(void)disable_irq_wake(bdi->irq);
...
static const struct i2c_device_id bq24190_i2c_ids[] = {
	...
	{ "bq24195", BQ24190_REG_VPRS_PN_24190 },
	{ },
};

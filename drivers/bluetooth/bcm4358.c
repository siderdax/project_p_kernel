...
static int bcm4358_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (!blocked) {
...
#ifdef BT_LPM_ENABLE
		if ( irq_set_irq_wake(btm_gpio.irq, 1)) {
		...
		gpio_set_value(btm_gpio.bt_wake, 1);
#endif
		gpio_set_value(btm_gpio.bt_en, 1);
		bt_is_running = 1;
		msleep(100);
...
#ifdef BT_LPM_ENABLE
		if (gpio_get_value(btm_gpio.bt_en) && irq_set_irq_wake(btm_gpio.irq, 0)) {
			pr_err("[BT] Release_irq_wake failed.\n");
			return -1;
		}
#endif
		bt_is_running = 0;
		gpio_set_value(btm_gpio.bt_en, 0);
	}
	return 0;
}
...
static void set_wake_locked(int wake)
{
	...
	gpio_set_value(btm_gpio.bt_wake, wake);
...
static irqreturn_t host_wake_isr(int irq, void *dev)
{
	...
	host_wake = gpio_get_value(btm_gpio.bt_hostwake);
...
static int bcm_bt_lpm_init(struct platform_device *pdev)
{
	...
	btm_gpio.irq = gpio_to_irq(btm_gpio.bt_hostwake);
	ret = request_irq(btm_gpio.irq, host_wake_isr, IRQF_TRIGGER_RISING,
		"bt_host_wake", NULL);
	if (ret) {
...
static int bcm4358_bluetooth_probe(struct platform_device *pdev)
{
	...
	btm_gpio.bt_en = of_get_gpio(pdev->dev.of_node, 0);

	if (!gpio_is_valid(btm_gpio.bt_en)) {
		pr_err("[BT] btm_gpio.bt_en get gpio failed.\n");
		return -EINVAL;
	}

	rc = gpio_request(btm_gpio.bt_en, "bten_gpio");

	if (unlikely(rc)) {
		pr_err("[BT] btm_gpio.bt_en request failed.\n");
		return rc;
	}

	btm_gpio.bt_wake =of_get_gpio(pdev->dev.of_node, 1);

	if (!gpio_is_valid(btm_gpio.bt_wake)) {
		pr_err("[BT] btm_gpio.bt_wake get gpio failed.\n");
		return -EINVAL;
	}

	rc = gpio_request(btm_gpio.bt_wake, "btwake_gpio");

	if (unlikely(rc)) {
		pr_err("[BT] btm_gpio.bt_wake request failed.\n");
		gpio_free(btm_gpio.bt_en);
		return rc;
	}

	btm_gpio.bt_hostwake =of_get_gpio(pdev->dev.of_node, 2);

	if (!gpio_is_valid(btm_gpio.bt_hostwake)) {
		pr_err("[BT] btm_gpio.bt_hostwake get gpio failed.\n");
		return -EINVAL;
	}

	rc = gpio_request(btm_gpio.bt_hostwake,"bthostwake_gpio");

	if (unlikely(rc)) {
		pr_err("[BT] btm_gpio.bt_hostwake request failed.\n");
		gpio_free(btm_gpio.bt_wake);
		gpio_free(btm_gpio.bt_en);
		return rc;
	}

	gpio_direction_input(btm_gpio.bt_hostwake);
	gpio_direction_output(btm_gpio.bt_wake, 0);
	gpio_direction_output(btm_gpio.bt_en, 0);
	...
	if (unlikely(!bt_rfkill)) {
		...
		gpio_free(btm_gpio.bt_hostwake);
		gpio_free(btm_gpio.bt_wake);
		gpio_free(btm_gpio.bt_en);
		return -ENOMEM;
	}

...
#ifdef PMIC_DEBUG
static struct kobject *pmic_status_kobj;
#endif
...
static ssize_t data_read(struct file *file, char __user *data, size_t count, loff_t *ppos)
{
	...
	struct s2mps15_info *info = rdev_get_drvdata(rdev);
	int reg_id = rdev_get_id(rdev);

	pr_debug("%s: count=%zu, ppos=%lld\n", __func__, count, *ppos);

	switch (info->opmode[reg_id] >> S2MPS15_ENABLE_SHIFT) {
		case SEC_OPMODE_OFF:
...
static ssize_t data_write(struct file *file, const char __user *data, size_t count, loff_t *ppos)
{
	...
	if (size > 0) {
		if (strncasecmp("OFF", buffer, sizeof("OFF") - 1) == 0) {
			info->opmode[reg_id] = SEC_OPMODE_OFF << S2MPS15_ENABLE_SHIFT;
		} else if (strncasecmp("SUSPEND", buffer, sizeof("SUSPEND") - 1) == 0) {
			info->opmode[reg_id] = SEC_OPMODE_SUSPEND << S2MPS15_ENABLE_SHIFT;
		} else if (strncasecmp("LOWPOWER", buffer, sizeof("LOWPOWER") - 1) == 0) {
			info->opmode[reg_id] = SEC_OPMODE_LOWPOWER << S2MPS15_ENABLE_SHIFT;
		} else if (strncasecmp("ON", buffer, sizeof("ON") - 1) == 0) {
			info->opmode[reg_id] = SEC_OPMODE_ON << S2MPS15_ENABLE_SHIFT;
		} else {
...
static ssize_t all_data_read(struct file *file, char __user *data, size_t count, loff_t *ppos)
{
	int i;
	ssize_t size;
	struct s2mps15_info *info = file->private_data;
	...
	for (i = 0; i < ARRAY_SIZE(info->rdev); i++) {
		...
		if (!IS_ERR_OR_NULL(rdev)) {
			...
			voltage = 0;
			mode = 0;
			ret = regmap_read(rdev->regmap, regulators[S2MPS15_DESC_TYPE0][id].enable_reg, &value);
			if (ret < 0) {
			...
			switch (info->opmode[id] >> S2MPS15_ENABLE_SHIFT) {
				case SEC_OPMODE_OFF:
...
static void initialize_debug_fs(struct s2mps15_info *info) {
	struct dentry *root, *reg_dir;
	int i;

	pr_debug("%s is called.\n", __func__);

	root = debugfs_create_dir("s2mps15", NULL);
	if (IS_ERR(root)) {
...
#endif
#ifdef PMIC_DEBUG
/*add sysfs to check register*/
static ssize_t pmic_show_dummy1(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return 0;
}

static ssize_t pmic_store_set_reg(struct kobject *kobj,
			struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	for (idx = 0; idx < count; idx++) {
		if (buf_temp[idx] == ' ' || buf_temp[idx] == '	') {
			buf_temp[idx] = '\0';
			break;
		}
	}
	
	sscanf(buf_temp, "%x", &reg);
	sscanf(&buf_temp[idx+1], "%x", &value);
	...
	sec_reg_write(static_info->iodev, reg1, value1);

	msleep(200);

	sec_reg_read(static_info->iodev, reg1, &value1);

	printk("set result: reg 0x%x = 0x%x\n", reg1, value1);

	return count;
	
}

static ssize_t pmic_show_dummy2(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
#if 0
	struct regulator *power; 
	
	power = regulator_get(NULL, "vqmmc");
	regulator_set_voltage(power, 1800000, 1800000);
	regulator_put(power);

	power = regulator_get(NULL, "vdd_sim0_cp");
	regulator_set_voltage(power, 3000000, 3000000);
	regulator_enable(power);
	regulator_put(power);

	power = regulator_get(NULL, "vdd_arm");
	regulator_set_voltage(power, 1050000, 1050000);
	regulator_put(power);

	power = regulator_get(NULL, "vdd_int");
	regulator_set_voltage(power, 1050000, 1050000);
	regulator_put(power);

	power = regulator_get(NULL, "vdd_g3d");
	regulator_set_voltage(power, 1050000, 1050000);
	regulator_put(power);

	power = regulator_get(NULL, "vdd_mif");
	regulator_set_voltage(power, 1050000, 1050000);
	regulator_put(power);	
...
static ssize_t pmic_store_get_reg(struct kobject *kobj,
			struct kobj_attribute *attr, const char *buf, size_t count)
{
	int reg;
	u8 reg1, value1;
	...
	if (reg != 0xff) {
		sec_reg_read(static_info->iodev, reg1, &value1);
		printk("reg 0x%x value is 0x%x\n", reg1, value1);
	} else {
		/* print all the register staus */
		for (reg1 = S2MPS15_REG_ID; reg1 <= S2MPS15_REG_LDO_DSCH4; reg1++) {
			sec_reg_read(static_info->iodev, reg1, &value1);
...
static struct kobj_attribute pmic_set_reg_attr  =__ATTR(setreg, 0644, pmic_show_dummy1, pmic_store_set_reg);
static struct kobj_attribute pmic_get_reg_attr  =__ATTR(getreg, 0644, pmic_show_dummy2, pmic_store_get_reg);
#endif
static int s2mps15_pmic_probe(struct platform_device *pdev)
{
	...
	if (ret) {
        ...
        initialize_debug_fs(s2mps15);
#ifdef PMIC_DEBUG
	pmic_status_kobj = kobject_create_and_add("pmic", NULL);
	if(!pmic_status_kobj)
		printk("failed to create pmic sysfs\n");
		
	ret = sysfs_create_file(pmic_status_kobj, &pmic_set_reg_attr.attr);
	if (ret)
		printk("Failed to create set_reg in sysfs\n");
	ret = sysfs_create_file(pmic_status_kobj, &pmic_get_reg_attr.attr);
	if (ret)
		printk("Failed to create get_reg in sysfs");
#endif	
	return 0;
err:

...
/*
static int mxt_plugin_hook_t81(struct plug_interface *pl, u8* msg);
static int mxt_plugin_hook_t92(struct plug_interface *pl, u8* msg);
static int mxt_plugin_hook_t93(struct plug_interface *pl, u8 *msg);
*/
...
static int mxt_bootloader_read(struct mxt_data *data, u8 *val, unsigned int count)
{
	...
	ret = __mxt_read(client,val,count);
	client->addr = addr;
	
	mutex_unlock(&data->dma_access_mutex);
	
	if (ret != count) {
		dev_err(&client->dev, "reads bootloader data error %d!!\n",ret);
		return -EFAULT;
	} 
	...
	unsigned int count)
{
	...
	/*
	if (!val)
		dev_info(&client->dev, "val null\n",val);
	else
		dev_info(&client->dev, "0x%x len %d\n",val[0],count);
	*/
	ret = __mxt_write(client,val,count);
	client->addr = addr;
	
	mutex_unlock(&data->dma_access_mutex);
	
	if (ret != count) {
		dev_err(&client->dev, "writes bootloader data error %d!!\n",ret);
		return -EFAULT;
	} 
...
retry_read:
	...
	ret = __mxt_read(client, val, len);
	if (ret != len) {
		dev_err(&client->dev, "reads data error!!\n");
		retval = -EFAULT;
		goto out;
	} 	
...
out:
	...
	if(retval) {
		if (!retry) {
			...
			dev_err(&client->dev, "%s: i2c send failed (%d) reg %d len %d\n",
...
retry_write:
	mutex_lock(&data->dma_access_mutex);
	ret = __mxt_write(client, buf, count);
	mutex_unlock(&data->dma_access_mutex);
	if (ret == count) {
		ret = 0;
	} else {
		if (!retry) {
			...
			dev_err(&client->dev, "%s: i2c send failed (%d) reg %d\n",
				__func__, ret, reg);
			ret = -EIO;
		}
	}
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		if (!retry) {
			...
			dev_err(&client->dev, "%s: i2c transfer failed (%d) reg %d\n",
				__func__, ret, reg);
			return -EIO;
		}
	}
...
mxt_get_object(struct mxt_data *data, u8 type)
{
	...
	dev_info2(&data->client->dev, "Invalid object type T%u\n", type);
	return NULL;
}
...
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	mxt_plugin_hook_t9(&data->plug, id, x, y, status);
#endif
}
...
extern int janeps_input_report(event_type evt_type, x_coord x, y_coord y);
#endif
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	data->update_input = true;
#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
	mxt_plugin_hook_t100(&data->plug, id, x, y, status);
#endif
}
...
static void mxt_proc_t15_messages(struct mxt_data *data, u8 *msg)
{
	...
	num_keys =data->pdata->num_keys[T15_KEY];
	if (!num_keys)
		return;
...
static void mxt_proc_t15_messages(struct mxt_data *data, u8 *msg)
{
	...
	num_keys =data->pdata->num_keys[T15_KEY];
	keymap = data->pdata->keymap[T15_KEY];
	for (key = 0; key < num_keys; key++) {
		curr_state = test_bit(key, &data->t15_keystatus);
		new_state = test_bit(key, &keystates);
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (info_crc == data->info_crc) {
		...
		if (mxt_plugin_cal_t37_check_and_calibrate(&data->plug, true,false) != 0)
#endif
	...
	while (data_pos < cfg->size - 16) {
		...
		if (size > mxt_obj_size(object)) {
			...
			dev_warn(dev, "Discarding %u byte(s) in T%u\n",
				 (int)(size - mxt_obj_size(object)), type);
			...
			dev_warn(dev, "Zeroing %u byte(s) in T%d\n",
				 (int)(mxt_obj_size(object) - size), type);
		}
				  ...
				  struct mxt_object *object_table)
{
	...
	for (i = 0; i < data->info->object_num; i++) {
		...
		dev_dbg(&data->client->dev,
			...
			(int)mxt_obj_size(object), (int)mxt_obj_instances(object),
			min_id, max_id);
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	int error;
#if 1
	data->reg_vddio = regulator_get(dev, data->pdata->regulator_vddio);
	if (IS_ERR(data->reg_vddio)){
		error = PTR_ERR(data->reg_vddio);
		dev_err(dev, "Error %d getting vddio regulator\n", error);
		goto fail;
	}
	data->reg_vdd = regulator_get(dev, data->pdata->regulator_vdd);
	...
	data->reg_avdd = NULL;
#else
	data->reg_vddio = regulator_get(dev, "vdd_tp_18v");
	if (IS_ERR(data->reg_vddio)) {
		error = PTR_ERR(data->reg_vddio);
		dev_err(dev, "Error %d getting vddio regulator\n", error);
		goto fail;
	}
	data->reg_vdd = regulator_get(dev, "vcc28_tp");
	if (IS_ERR(data->reg_vdd)) {
		error = PTR_ERR(data->reg_vdd);
		dev_err(dev, "Error %d getting vdd regulator\n", error);
		goto fail;
...
#if defined(CONFIG_MXT_SELFCAP_TUNE)
	//this will cost many time to finish, so put it in service excute
...
#ifdef CONFIG_OF
static struct mxt_platform_data *atmel_mxt540_parse_dt(struct i2c_client *client)
{
	struct mxt_platform_data *pdata;
	...
	pdata->gpio_irq = of_get_named_gpio(np, "irq_gpio", 0);
	client->irq = gpio_to_irq(pdata->gpio_irq);
	pdata->gpio_reset = of_get_named_gpio(np, "reset_gpio", 0);

	if (of_property_read_u32(np, "t15_num_keys", &pdata->t15_num_keys)) {
		dev_err(dev, "Failed to get irq_type property\n");
		return NULL;
	}

	if (of_property_read_string(np, "regulator_vddio", &pdata->regulator_vddio)) {
		dev_err(dev, "Failed to get regulator_dvdd name property\n");
		return NULL;
	}
	if (of_property_read_string(np, "regulator_vdd", &pdata->regulator_vdd)) {
		dev_err(dev, "Failed to get regulator_avdd name property\n");
		return NULL;
	}
...
#if 1
static int mxt_check_i2cbus(struct i2c_client *client)
{
	size_t size;
	void *id_buf;
	int error = 0;
				   ...
				   const struct i2c_device_id *id)
{
	...
	data->client = client;
	if (mxt_check_i2cbus(client)){
		dev_err(&client->dev, "mxt_check_i2cbus failed!\n");
		goto err_free_mem;
	}
#if defined(CONFIG_MXT_I2C_DMA)
	client->addr |= I2C_RS_FLAG | I2C_ENEXT_FLAG | I2C_DMA_FLAG;
	...
	if (!data->pdata && client->dev.of_node)
		data->pdata = atmel_mxt540_parse_dt(client);
#endif
	data->irq = client->irq;
	...
	dev_info(&client->dev, "Mxt probe finished\n");
		...
		error = request_irq(client->irq, mxt_interrupt_pulse_workqueue,
				data->pdata->irqflags,/*IRQF_TRIGGER_FALLING,data->pdata->irqflags IRQF_TRIGGER_LOW*/
				client->name, data);
		if (error) {

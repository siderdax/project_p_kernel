		...
		if (finger_status != OBJECT_NOT_PRESENT) {
			...
			if (rmi4_data->finger[finger].state) {
				...
				if (rmi4_data->finger[finger].state != finger_status)
					tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "[%d][C][%d] 0x%02x T[%d/%u] M[%d]"
				...
				tsp_debug_dbg(true, &rmi4_data->i2c_client->dev, "[%d][P][%d] 0x%02x T[%d/%u]"
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
					", x = %d, y = %d, wx = %d, wy = %d\n",	finger, rmi4_data->board->device_num, finger_status, tool_type,
...
//	synaptics_rmi4_stop_device(rmi4_data); // for bring-up

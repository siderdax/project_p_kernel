...
int __init dhd_wlan_init_gpio(void)
{
	...
	if (!root_node) {
		WARN(1, "failed to get device node of bcm4339\n");
		return -ENODEV;
	}
#if 1
	/* ========== WLAN_PWR_EN ============ */
	...
	printk(KERN_INFO "%s: ===========HAHAHA==============================ekekekekekek===>start\n", __FUNCTION__);
	wlan_host_wake_irq = gpio_to_irq(wlan_host_wake_up);
	printk(KERN_INFO "%s: wlan_host_wake_irq =%d wlan_host_wake_up=%d\n", __FUNCTION__,wlan_host_wake_irq, wlan_host_wake_up);
...
#if defined(CONFIG_MACH_MV7420)
	argos_irq_affinity_setup_label(IRQ_SPI(202), "WIFI", affinity_cpu_mask, default_cpu_mask);
	argos_irq_affinity_setup_label(IRQ_SPI(16), "WIFI", affinity_cpu_mask, default_cpu_mask);
#endif
}
#endif /* CONFIG_ARGOS */
